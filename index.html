...
// Ícones modernos
import { LogOut, UserPlus, LogIn, Save, Trash2, Loader, Edit, Upload, DollarSign, HardHat, ClipboardList, Image } from 'lucide-react';
...
const App = () => {
    ...
    const deletarRecibo = async (id, url) => {
        ...
        try {
            const userId = user.uid;
            // Primeiro deleta o documento no Firestore
            const docRef = doc(db, 'artifacts', appId, 'users', userId, 'recibos', id);
            await deleteDoc(docRef);
            
            // Nota: Para deletar o arquivo no Storage, seria necessário uma Cloud Function
            // pois o cliente não tem permissão para deletar arquivos diretamente
        } catch (e) {
                                            ...
                                            <div key={recibo.id} className="bg-gray-700 rounded-lg overflow-hidden border border-gray-600 shadow-md">
                                                <div className="p-4">
                                                    <div className="flex justify-between items-start mb-2">
                                                        ...
                                                        <button
                                                            onClick={() => deletarRecibo(recibo.id, recibo.url)}
